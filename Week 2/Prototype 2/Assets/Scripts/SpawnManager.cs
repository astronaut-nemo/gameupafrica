...
public class SpawnManager : MonoBehaviour
{
    ...
    // SpawnRandomAnimal is a method that spawns random animals (like the name says ( ˙▿˙ ) )
    void SpawnRandomAnimal()
    {
        // Local variables
        int animalIndex = Random.Range(0, animalPrefabs.Length); // Randomly select an index in the array
        Vector3 spawnPos = new Vector3(Random.Range(-spawnRangeX, spawnRangeX), 0, spawnPosZ); // Randomise the horizontal position within the range
